...
User.getInitialProps = wrapper.getInitialPageProps(store => async(ctx) => {
    ...
    return {
        data: {
            list: await getUsers({skip: 0, legalObject: ctx.query.id, ...store.getState().app.branch?{branch: store.getState().app.branch._id}:{}}, ctx.req?await getClientGqlSsr(ctx.req):undefined),
            count: await getUsersCount({legalObject: ctx.query.id, ...store.getState().app.branch?{branch: store.getState().app.branch._id}:{}}, ctx.req?await getClientGqlSsr(ctx.req):undefined),
        }
    };
});

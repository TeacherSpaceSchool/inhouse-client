...
import {getTypeCharacteristics, getTypeCharacteristicsCount, addTypeCharacteristic, setTypeCharacteristic, deleteTypeCharacteristic, getUnloadTypeCharacteristics, uploadTypeCharacteristic} from '../src/gql/typeCharacteristic'
...
const TypeCharacteristics = React.memo((props) => {
    ...
    const getList = async ()=>{
        setList(cloneObject(await getTypeCharacteristics({search, skip: 0})));
        setCount(await getTypeCharacteristicsCount({search}));
    ...
    const checkPagination = async()=>{
        if(paginationWork.current){
            let addedList = cloneObject(await getTypeCharacteristics({skip: list.length, search}))
    ...
    //render
    return (
        <App unsaved={unsaved} checkPagination={checkPagination} searchShow={true} pageName='Типы характеристик' menuItems={menuItems} anchorElQuick={anchorElQuick} setAnchorElQuick={setAnchorElQuick}>
            <Head>
                <title>Типы характеристик</title>
                <meta name='description' content='Inhouse.kg | МЕБЕЛЬ и КОВРЫ БИШКЕК' />
                <meta property='og:title' content='Типы характеристик' />
                <meta property='og:description' content='Inhouse.kg | МЕБЕЛЬ и КОВРЫ БИШКЕК' />
                <meta property='og:type' content='website' />
                <meta property='og:image' content={`${urlMain}/512x512.png`} />
                <meta property='og:url' content={`${urlMain}/typecharacteristics`} />
                <link rel='canonical' href={`${urlMain}/typecharacteristics`}/>
            </Head>
                                            ...
                                            <MenuItem onClick={()=>{
                                                if(newElement.name.length) {
                                                    setMiniDialog('Вы уверены?', <Confirmation action={async ()=>{
                                                        let res = await addTypeCharacteristic(newElement)
                                                ...
                                                [
                                                    <MenuItem sx={{marginBottom: '5px'}} key='MenuItem1' onClick={async()=>{
                                                        if(element.name.length) {
                                                            setMiniDialog('Вы уверены?', <Confirmation action={async () => {
                                                                let res = await setTypeCharacteristic(element)
                                                                if(res==='OK') {
                                                                    showSnackBar('Успешно', 'success')
                                                    ...
                                                    data.deleted?
                                                        <MenuItem key='MenuItem3' onClick={async()=>{
                                                            setMiniDialog('Вы уверены?', <Confirmation action={async () => {
                                                                let res = await deleteTypeCharacteristic(element._id)
                                                                if(res==='OK'){
                                                                    showSnackBar('Успешно', 'success')
                                                                    delete unsaved.current[list[idx]._id]
                                                                    let _list = [...list]
            ...
            {
                data.add||data.edit?
                    <UnloadUpload upload={uploadTypeCharacteristic} uploadText={uploadText} unload={()=>getUnloadTypeCharacteristics({search})}/>
                    :
                    null
            }
        </App>
...
TypeCharacteristics.getInitialProps = wrapper.getInitialPageProps(store => async(ctx) => {
    ...
    return {
        data: {
            ...
            list: cloneObject(await getTypeCharacteristics({skip: 0},  ctx.req?await getClientGqlSsr(ctx.req):undefined)),
            count: await getTypeCharacteristicsCount({}, ctx.req?await getClientGqlSsr(ctx.req):undefined),
        }
    };
})
...
export default connect(mapStateToProps, mapDispatchToProps)(TypeCharacteristics);
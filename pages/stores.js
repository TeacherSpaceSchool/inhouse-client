...
const Store = React.memo((props) => {
    ...
    const getList = async ()=>{
        setList(cloneObject(await getStores({search, skip: 0})));
        setCount(await getStoresCount({search}));
    ...
    const checkPagination = async()=>{
        if(paginationWork.current){
            let addedList = cloneObject(await getStores({skip: list.length, search}))
    ...
    return (
        <App unsaved={unsaved} checkPagination={checkPagination} searchShow={true} pageName='Магазины' menuItems={menuItems} anchorElQuick={anchorElQuick} setAnchorElQuick={setAnchorElQuick}>
            <Head>
                ...
                <meta property='og:url' content={`${urlMain}/stores`} />
                <link rel='canonical' href={`${urlMain}/stores`}/>
            </Head>
                                            ...
                                            <MenuItem onClick={()=>{
                                                if(newElement.name.length) {
                                                    setMiniDialog('Вы уверены?', <Confirmation action={async ()=>{
                                                        let res = await addStore(newElement)
                                                ...
                                                [
                                                    <MenuItem sx={{marginBottom: '5px'}} key='MenuItem1' onClick={async()=>{
                                                        if(element.name.length) {
                                                            setMiniDialog('Вы уверены?', <Confirmation action={async () => {
                                                                let res = await setStore(element)
                                                                if(res==='OK') {
                                                                    showSnackBar('Успешно', 'success')
                                                    ...
                                                    data.deleted?
                                                        <MenuItem key='MenuItem3' onClick={async()=>{
                                                            setMiniDialog('Вы уверены?', <Confirmation action={async () => {
                                                                let res = await deleteStore(element._id)
                                                                if(res==='OK'){
                                                                    showSnackBar('Успешно', 'success')
                                                                    delete unsaved.current[list[idx]._id]
                                                                    let _list = [...list]
...
Store.getInitialProps = wrapper.getInitialPageProps(store => async(ctx) => {
    ...
    return {
        data: {
            ...
            list: cloneObject(await getStores({skip: 0},  ctx.req?await getClientGqlSsr(ctx.req):undefined)),
            count: await getStoresCount({}, ctx.req?await getClientGqlSsr(ctx.req):undefined),
        }
    };
})
